...
import {
    toogleNav, hideVacayList, toogleSearch, allVacationsCall, searchRequest, soundCloudIcon, toogleGSearch,
    toogleGNav, hideShowGVacayList, GsoundCloudIcon
} from '../actions/action';
import $ from 'jquery';
                ...
                {this.props.arrayofVacays.map((vacation, index) => {
                    return (
                        <div key={index} onClick={(event) => {
                            this.handleVacationClicked(vacation)
                        }} className="vacayContainer">
